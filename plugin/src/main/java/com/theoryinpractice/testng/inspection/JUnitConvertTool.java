...
public class JUnitConvertTool extends BaseJavaLocalInspectionTool {
    ...
    private static final LocalizeValue DISPLAY_NAME = LocalizeValue.localizeTODO("Convert JUnit Tests to TestNG");
    public static final LocalizeValue QUICKFIX_NAME = LocalizeValue.localizeTODO("Convert TestCase to TestNG");
    ...
    public ProblemDescriptor[] checkClass(
        ...
        if (TestNGUtil.inheritsJUnitTestCase(psiClass) || TestNGUtil.containsJunitAnnotations(psiClass)) {
            ...
            ProblemDescriptor descriptor =
                manager.createProblemDescriptor(nameIdentifier != null ? nameIdentifier : psiClass, "TestCase can be converted to TestNG",
    ...
    public static class JUnitConverterQuickFix implements LocalQuickFix {
        ...
        public void applyFix(@Nonnull Project project, @Nonnull ProblemDescriptor descriptor) {
            ...
            if (!TestNGUtil.checkTestNGInClasspath(psiClass)) {
                return;
            }
            try {
                    ...
                    for (PsiMethodCallExpression methodCall : methodCalls) {
                        ...
                        if ("assertTrue".equals(methodName) || "assertFalse".equals(methodName)) {
                            ...
                            if ("assertFalse".equals(methodName) && assertStatement != null) {
                                ...
                                LOG.assertTrue(assertCondition != null);
                                assertCondition.replace(factory.createExpressionFromText(
                                    "!(" + assertCondition.getText() + ')',
                            ...
                            if (expressions.length == 2) {
                                inserted = methodCallStatement.replace(factory.createStatementFromText(
                                    "org.testng.Assert." + methodCall.getText() + ";",
                                    methodCall.getParent()
                                ));
                            }
                            else if (expressions.length == 3) {
                                String call = "org.testng.Assert." + methodName + '(' + expressions[2].getText() + ", " +
                                    expressions[1].getText() + ", " + expressions[0].getText() + ");";
        ...
        private static void addMethodJavadocLine(PsiElementFactory factory, PsiMethod method, String javaDocLine)
        ...
        private static void addMethodAnnotations(PsiElementFactory factory, PsiMethod method) throws IncorrectOperationException {
            ...
            if (annotation != null) {
                JavaCodeStyleManager.getInstance(annotation.getProject())
                    .shortenClassReferences(method.getModifierList().addAfter(annotation, null));
            }
        }
