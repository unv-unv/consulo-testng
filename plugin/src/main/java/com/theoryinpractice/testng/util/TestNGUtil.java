...
public class TestNGUtil {
    public static final LocalizeValue TESTNG_GROUP_NAME = LocalizeValue.localizeTODO("TestNG");
    ...
    public static boolean hasTest(PsiModifierListOwner element, boolean checkHierarchy, boolean checkDisabled, boolean checkJavadoc) {
        ...
        boolean hasAnnotation =
            AnnotationUtil.isAnnotated(element, TEST_ANNOTATION_FQN, checkHierarchy ? AnnotationUtil.CHECK_HIERARCHY : 0);
        if (hasAnnotation) {
            if (checkDisabled) {
        ...
        if (element instanceof PsiClass) {
            ...
            if (psiClass != null) {
                final PsiAnnotation annotation = checkHierarchy ? AnnotationUtil.findAnnotationInHierarchy(
                    psiClass,
                    Collections.singleton(TEST_ANNOTATION_FQN)
                ) : AnnotationUtil.findAnnotation
                    (psiClass, true, TEST_ANNOTATION_FQN);
                if (annotation != null) {
        ...
        if (psiMethods != null) {
            for (final PsiMethod psiMethod : psiMethods) {
                ApplicationManager.getApplication()
                    .runReadAction(() -> appendAnnotationAttributeValues(
                        results,
                        AnnotationUtil.findAnnotation(psiMethod, test),
                        psiMethod
                    ));
            }
        }
    ...
    public static boolean inheritsITestListener(@NotNull PsiClass psiClass) {
        ...
        final PsiClass aListenerClass =
            JavaPsiFacade.getInstance(project).findClass(ITestNGListener.class.getName(), GlobalSearchScope.allScope(project));
    ...
    public static boolean isTestngXML(final VirtualFile virtualFile) {
        if ("xml".equalsIgnoreCase(virtualFile.getExtension()) && virtualFile.isInLocalFileSystem() && virtualFile.isValid()) {
            ...
            try {
                result =
                    NanoXmlUtil.parseHeader(new InputStreamReader(new BufferExposingByteArrayInputStream(virtualFile.contentsToByteArray())))
                        .getRootTagLocalName();
            }
            catch (IOException e) {
                return false;
    ...
    public static Version detectVersion(Project project, @Nullable Module module) {
        ...
        return CachedValuesManager.getManager(project).getCachedValue(module, () ->
        {
            ...
            PsiClass aClass =
                psiFacade.findClass("org.testng.internal.Version", GlobalSearchScope.moduleWithDependenciesAndLibrariesScope(module));
            if (aClass != null) {
